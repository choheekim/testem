...
module.exports = class App extends EventEmitter {
  ...
  onBrowserRelogin(browserName, id, socket) {
    let browser = find(this.runners, runner => {
      console.log(`runner.launcherId:${runner.launcherId} === id:${id} ? ${runner.launcherId === id}`)
      log.info(`runner.launcherId:${runner.launcherId} === id:${id} ? ${runner.launcherId === id}`)
      if (runner.launcherId === id) {
        log.info('existing socket')
        log.info(util.inspect(runner.socket))
        log.info('incoming socket')
        log.info(util.inspect(socket))
        console.log('existing socket')
        console.log(util.inspect(runner.socket))
        console.log('incoming socket')
        console.log(util.inspect(socket))
      }
      return runner.launcherId === id && runner.socket;
    });
    log.info(`browser found from id=${id} browserName=${browserName}? ${browser}`)
    if (!browser) {
  ...
  onBrowserLogin(browserName, id, socket) {
    let browser = find(this.runners, runner => {
      log.info(`runner.launcherId:${runner.launcherId} === id:${id} ? ${runner.launcherId === id}`)
      if (runner.launcherId === id && runner.socket) {
        log.info('existing socket')
        log.info(util.inspect(runner.socket))
        log.info('incoming socket')
        log.info(util.inspect(socket))
    ...
    if (!browser) {
      ...
      log.info(`create new browser test runner for id=${id} browserName=${browserName}`)
