...
module.exports = class BrowserTestRunner {
  ...
  onAfterTests() {
    log.info('browser_test_runner: onAfterTests')
    this.finish();
  }
  ...
  onGlobalError(msg, url, line) {
    ...
    if (config.get('bail_on_uncaught_error')) {
      ...
      log.info('browser_test_runner: onGlobalError');
  ...
  onProcessExit(code) {
    log.info('browser_test_runner: onProcessExit');
  ...
  onProcessError(err) {
    log.info('browser_test_runner: onProcessError');
    let browserProcess = this.process;
    this.process = null;
  ...
  finish() {
    log.info('browser_test_runner: finish');
    if (this.finished) { return; }
    ...
    return this.exit().then(() => {
      log.info('browser_test_runner: in finish.')
